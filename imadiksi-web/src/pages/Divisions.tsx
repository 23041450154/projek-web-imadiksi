...
export default function Divisions() {
    const { divisions, organizationMembers, loading } = useData();

    // Get members for a specific division from organization_members table
    const getDivisionMembers = (divisionId: string) => {
        return organizationMembers
            .filter(m => m.division_id === divisionId && m.is_active !== false)
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
                    {divisions.map((div) => {
                        ...
                        const divMembers = getDivisionMembers(div.id!);
                        ...
                        return (
                            <Card key={div.id} className="hover:border-primary-200 dark:hover:border-primary-800 transition-colors">
                                ...
                                {/* Show members from organization_members table */}
                                {divMembers.length > 0 && (
                                        ...
                                        <div className="space-y-2">
                                            {divMembers.map((member) => (
                                                <div key={member.id} className="flex items-center gap-3 text-sm">
                                                    {member.photo_url ? (
                                                        <img src={member.photo_url} alt={member.name} className="w-8 h-8 rounded-full object-cover" />
                                                    ) : (
